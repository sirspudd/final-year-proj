...
{	//Constants
	...
	//MELAX WELL IS DEFINED AS SUCH AND THESE VALUES SHOULD NOT BE CHANGED
	
	private final int FULLWIDTH = 6;
	private final int CONCEPTUALWIDTH = 6;
	...
	private final boolean HEIGHTLIMITED = true;
	...
	*/	
	...
	{	
		...
		//Following reduction check height reduce to height 2 if above and punish accordingly
		while(getResultingHeight() > 2)
		{	//Punishing for raising height
			reward += -100;
			score++;
			 
			for(int hght2 = (HEIGHT-1);hght2 > 0;hght2--)
				for(int wdth2 = 0;wdth2 < WIDTH;wdth2++)
				{	
					blocks[hght2*WIDTH + wdth2] = blocks[(hght2-1)*WIDTH + wdth2];
				}
