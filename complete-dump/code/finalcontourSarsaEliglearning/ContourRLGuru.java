...
{	//constants
	...
	private final double ELIGFACTOR = 0.5;
	...
	{	
		...
		STATEACTIONSPACE = CONTOURSPACE * NUMTETROMINOS * ORIENTATIONS * REDUCEDWIDTH; 
		
		Q = new double[STATEACTIONSPACE]; 
		
		elig = new double[STATEACTIONSPACE]; 
	...
	public void flushPlayer()
	{
		for(int count = 0;count < STATEACTIONSPACE;count++)
			Q[count] = randNo.nextDouble();
			//Q[count] = STARTINGVALUE;
	}
	...
	{	
		double nextValue = Q[nextHashCode];
		
		//defined as such in Sutton & Barto
		if (theGame.isDead()) nextValue = 0;
		
		double delta = reward + GAMMA*nextValue - Q[currentHashCode];
		
		elig[currentHashCode] = 1;
		
		for(int count = 0; count < STATEACTIONSPACE; count++)
		{
			Q[count] = Q[count] + ALPHA*delta*elig[count];
			elig[count] = elig[count]*ELIGFACTOR;
		}
	}
	...
	{	int hash = 0;
		...
		{	//VIP, learning only occurs in wells with WIDTH == REDUCEDWIDTH
			...
			//This is the hash Value of the contour
			...
			{	
				...
				{	
					...
					{	//This is the term that limits it to WIDTH 4
						...
						oStream.println("" + (count-MAXHEIGHTDIFF) + "\t" + (outer-MAXHEIGHTDIFF) + "\t" + (inner-MAXHEIGHTDIFF) + " : " + Q[tempInt]);
	...
	{	//prints out values in value table
		for(int count = 0; count < STATEACTIONSPACE; count++)
			if (Q[count] != STARTINGVALUE) System.out.print(" " + Q[count]);
		System.out.println();
	}
	...
	{	
		...
		//System.out.println("Final hash : " + (((hash + CONTOURSPACE*tetType) + CONTOURSPACE*NUMTETROMINOS*pos) + CONTOURSPACE*NUMTETROMINOS*REDUCEDWIDTH*rot));
		//System.out.println();
		
		return (((hash + CONTOURSPACE*tetType) + CONTOURSPACE*NUMTETROMINOS*pos) + CONTOURSPACE*NUMTETROMINOS*REDUCEDWIDTH*rot);
	}
