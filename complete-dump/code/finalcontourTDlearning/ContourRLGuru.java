...
{	//constants
	...
	{
		for(int count = 0;count < CONTOURSPACE;count++)
	...
	{	
		...
		{	//actual learning can only occur in wells where the virtual and real width agree
				learn(currentHashCode,nextHashCode,transition.reward);
		}
	}
	...
	{	
		...
		transition.value = GAMMA*transition.value + tempTransition.reward;
	}
	...
	{	int hash = 0;
		...
		{	//VIP, learning only occurs in wells with WIDTH == REDUCEDWIDTH
			...
			hash = mirrorHash(testGame.getBlocks());
	...
	{	//Could be used for expansion of TD Player
